...
Client-side implementations assume a single user context. Amplitude Experiment loads flag and experiment configurations on startup for use with [local evaluation](./general/evaluation/local-evaluation.md), and  fetches [remote evaluation](./general/evaluation/remote-evaluation.md) variants dynamically from Amplitude's remote evaluation service. When a variant is accessed from the SDK, an [exposure event](./general/experiment-event-tracking.md#exposure-events) is tracked for the user.
